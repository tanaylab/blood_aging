...
fig2 <- function() {
  ...
  correction.factor.df = read.table(CORRECTION.FACTOR.PATH, stringsAsFactors=F)
  correction.factor = correction.factor.df$factor
  names(correction.factor) = correction.factor.df$gene
...
de.analysis <- function(our.cdata, our.cdata.x, our.mdata, our.legc, our.annotation, our.assign, cell.to.samp, indiv.to.used.samp, fig.dir=BASE.FIG.DIR) {
  ...
  for (i in 1:2) {
    ...
    #ds.thresh=0.2
    #rs.thresh=0.28
    #genes.to.fix = names(correction.factor)[correction.factor >= -ds.thresh & correction.factor <= rs.thresh]
    #cur.samps.legc[genes.to.fix, samp.to.is.ultima[colnames(cur.samps.legc)]] = cur.samps.legc[genes.to.fix, samp.to.is.ultima[colnames(cur.samps.legc)]] + correction.factor[genes.to.fix]
